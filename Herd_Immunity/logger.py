...
class Logger(object):
    ...
    def write_metadata(self, pop_size, vacc_percentage, virus_name, mortality_rate,
        ...
        #print("Pop_size: "+str(pop_size)+"\tVacc%: "+ str(vacc_percentage)+"\tvirus_name: "+ virus_name+"\tMortality_Rate: "+str(mortality_rate)+"\tbasic_repro_num: "+str(basic_repro_num)+"\n")


        self.file.write("Pop_size: "+str(pop_size)+"\tVacc%: "+ str(vacc_percentage)+"\tvirus_name: "+ virus_name+"\tMortality_Rate: "+str(mortality_rate)+"\tbasic_repro_num: "+str(basic_repro_num)+"\n")
    ...
    def log_interaction(self, person1, person2, did_infect=None,
        ...
        if(did_infect):
            self.file.write("Person with ID:"+str(person1._id)+" infected Person with ID:"+str(person2._id)+"\n")
        elif(person2_vacc):
            self.file.write("Person with ID:"+str(person1._id)+" was not able to infect Person with ID:"+str(person2._id)+" because they are vaccinated\n")
        elif(person2_sick):
            self.file.write("Person with ID:"+str(person1._id)+" was not able to infect Person with ID:"+str(person2._id)+" because they were sick to begin with :o\n")
        else:
            self.file.write("Person with ID:"+str(person1._id)+" was not able to infect Person with ID:"+str(person2._id)+"\n")
    def log_infection_survival(self, person, did_die_from_infection):
        ...
        if did_die_from_infection:
            #print("Person with ID: " + str(person._id) + " dies.\n")
            self.file.write("Person with ID: " + str(person._id) + " dies.\n")
        else:
            #print("Person with ID: " + str(person._id) + " survived.\n")
            self.file.write("Person with ID: " + str(person._id) + " survived.\n")
    ...
    def log_time_step(self, time_step_number):
        ...
        # event logged ends up on a separate line!
        self.file = open(self.file_name,"a")
        if self.time_step_tracker:
            #print("Time Step: "+ str(time_step_number)+" ended.\n")
            self.file.write("Time Step: "+ str(time_step_number)+" ended.\n")
            self.time_step_tracker = False
            pass
        else:
            #print("Time Step: "+ str(time_step_number)+" began.\n")
            self.file.write("Time Step: "+ str(time_step_number)+" began.\n")
            self.time_step_tracker = True
            pass
    def daEndBaby(self,theStringOfWordsIWIllUseToEndThisSuperLongProgramThatTookWayToLong):
        self.file = open(self.file_name,"a")
        self.file.write("\n"+theStringOfWordsIWIllUseToEndThisSuperLongProgramThatTookWayToLong)
#def test():
    #filee = Logger("test.txt")
#test()
